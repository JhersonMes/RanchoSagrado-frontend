...
@Component({
  ...
  styleUrl: './ingredient-category.component.css',
})
export class IngredientCategoryComponent {
  ...
  protected $dataSource = signal(new MatTableDataSource<IngredientCategory>());
  protected $sort = viewChild(MatSort);
  protected $items = this.service.$listChange;
  protected readonly pageable = new PageableSearch<IngredientCategory>(
    (page, size) => this.service.findAllPageable(page, size),
    () => this.$items(),
  );
  protected displayedColumns = ['idCategory', 'name', 'actions'];
  ...
  delete(id: number) {
    if (window.confirm('¿Eliminar esta categoría de ingrediente?')) {
